...
- LMS at https://new.app.turnthebus.org
- CMS at https://studio.new.app.turnthebus.org
- CDN at https://cdn.new.app.turnthebus.org linked to a public read-only S3 bucket named new-ttb-india-storage
- Grafana at https://grafana.new.app.turnthebus.org/login
  - user: admin
  - pwd: prom-operator
- public ssh access via a t2.micro Ubuntu 20.04 LTS bastion EC2 instance at bastion.new.app.turnthebus.org
- daily data backups archived into a private S3 bucket named live-ttb-india-mongodb-backup
...
- LMS at https://dev.new.app.turnthebus.org
- CMS at https://studio.dev.new-app.turnthebus.org
- CDN at https://cdn.dev.new.app.turnthebus.org linked to an S3 bucket named dev-ttb-india-storage
...
.. code-block:: shell
  ...
  ssh bastion.new.app.turnthebus.org -i path/to/ttb-india-live-bastion.pem

  mysql -h mysql.new.app.turnthebus.org -u root -p

  mongo --port 27017 --host mongo.master.new.app.turnthebus.org -u root -p
  mongo --port 27017 --host mongo.reader.new.app.turnthebus.org -u root -p

  redis-cli -h redis.primary.new.app.turnthebus.org -p 6379
...
- **MySQL**. uses `AWS RDS <https://aws.amazon.com/rds/>`_ for all MySQL data, accessible inside the vpc as mysql.new.app.turnthebus.org:3306. Instance size settings are located in the `environment configuration file <terraform/environments/live/env.hcl>`_, and other common configuration settings `are located here <terraform/environments/live/rds/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **MongoDB**. uses `AWS DocumentDB <https://aws.amazon.com/documentdb/>`_ for all MongoDB data, accessible insid the vpc as mongodb.master.new.app.turnthebus.org:27017 and mongodb.reader.new.app.turnthebus.org. Instance size settings are located in the `environment configuration file <terraform/environments/live/env.hcl>`_, and other common configuration settings `are located here <terraform/modules/documentdb>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **Redis**. uses `AWS ElastiCache <https://aws.amazon.com/elasticache/>`_ for all Django application caches, accessible inside the vpc as cache.new.app.turnthebus.org. Instance size settings are located in the `environment configuration file <terraform/environments/live/env.hcl>`_. This is necessary in order to make the Open edX application layer completely ephemeral. Most importantly, user's login session tokens are persisted in Redis and so these need to be accessible to all app containers from a single Redis cache. Common configuration settings `are located here <terraform/environments/live/redis/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
